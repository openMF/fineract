...
import static org.apache.fineract.cob.savings.SavingsCOBConstant.JOB_NAME;
...
public class SavingsCOBManagerConfiguration {
    ...
    @Bean
    @StepScope
    public SavingsCOBPartitioner partitioner() {
        return new SavingsCOBPartitioner(propertyService, cobBusinessStepService, retrieveSavingsIdService, jobOperator, jobExplorer,
                SavingsCOBConstant.NUMBER_OF_DAYS_BEHIND);
    }

    @Bean
    public Step savingsCOBStep() {
        return stepBuilderFactory.get(SavingsCOBConstant.SAVINGS_COB_PARTITIONER_STEP)
                .partitioner(SavingsCOBConstant.SAVINGS_COB_WORKER_STEP, partitioner())
    ...
    public Step stayedLockedStep() {
        return new StepBuilder("Stayed locked savings accounts - Step", jobRepository).tasklet(stayedLockedTasklet(), transactionManager)
                .build();
    }
    ...
    public Job savingsCOBJob() {
        return new JobBuilder(JobName.SAVINGS_COB.name(), jobRepository) //
                ...
                .next(savingsCOBStep()).next(stayedLockedStep()) //
    ...
    public ExecutionContextPromotionListener customJobParametersPromotionListener() {
        ...
        listener.setKeys(new String[] { COBConstant.BUSINESS_DATE_PARAMETER_NAME, COBConstant.IS_CATCH_UP_PARAMETER_NAME });
        return listener;
    }
}
