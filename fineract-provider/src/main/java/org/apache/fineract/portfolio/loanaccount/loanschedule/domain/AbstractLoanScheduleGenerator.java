            ...
            for (LoanRepaymentScheduleInstallment installment : processInstallmentsInstallments) {
                ...
                for (Map.Entry<LocalDate, Money> disburseDetail : disburseDetailMap.entrySet()) {
                    if (disburseDetail.getKey().isAfter(installment.getFromDate())
                        ...
                        // disbursement detail
                        Money remainingPrincipal = disburseDetail.getValue().minus(downPaymentAmt);
                        loanScheduleParams.addOutstandingBalance(remainingPrincipal);
                        outstandingBalance = loanScheduleParams.getOutstandingBalance();
                        loanScheduleParams.addPrincipalToBeScheduled(remainingPrincipal);
                        principalToBeScheduled = loanScheduleParams.getPrincipalToBeScheduled();
                        loanApplicationTerms.setPrincipal(loanApplicationTerms.getPrincipal().plus(remainingPrincipal));
