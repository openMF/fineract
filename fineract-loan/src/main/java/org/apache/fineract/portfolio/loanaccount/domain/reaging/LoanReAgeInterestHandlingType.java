...
public enum LoanReAgeInterestHandlingType {
    ...
    EQUAL_AMORTIZATION("Equal Amortization of Outstanding amount"), //
    ;

    private final String humanReadableName;

    public static List<StringEnumOptionData> getValuesAsEnumOptionDataList() {
        return Arrays.stream(values()).map(v -> new StringEnumOptionData(v.name(), v.name(), v.getHumanReadableName())).toList();
    }

    public StringEnumOptionData asEnumOptionData() {
        return new StringEnumOptionData(name(), name(), getHumanReadableName());
    }
