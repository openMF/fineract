...
public class ProgressiveLoanTransactionValidatorImpl implements ProgressiveLoanTransactionValidator {
    ...
    public void validateBuyDownFeeEligibility(Loan loan) {
        if (!loan.getLoanProduct().getLoanProductRelatedDetail().isEnableBuyDownFee()) {
            throw new LoanTransactionProcessingException("Buy Down fees are not enabled for this loan product");
        }

        if (!loan.getLoanProduct().isProgressiveLoanProduct()) {
            throw new LoanTransactionProcessingException("Buy Down fees are only supported for progressive loans");
        }
