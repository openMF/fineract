...
public class GlobalConfigurationHelper {
    ...
    private void switchAndSetGlobalConfiguration(String configKey, boolean enabled, Long value) {
        GlobalConfigurationPropertyData configuration = ok(() -> fineractClient.globalConfiguration().retrieveOneByName(configKey));
        ...
        ok(() -> fineractClient.globalConfiguration().updateConfiguration1(configId, updateRequest));
        GlobalConfigurationPropertyData updatedConfiguration = ok(() -> fineractClient.globalConfiguration().retrieveOneByName(configKey));
    ...
    public GlobalConfigurationPropertyData getGlobalConfiguration(String configKey) {
        return ok(() -> fineractClient.globalConfiguration().retrieveOneByName(configKey));
    }
}
