...
public class DelinquencyGlobalInitializerStep implements FineractGlobalInitializerStep {
    ...
    public void setDefaultDelinquencyRanges() {
        ...
        try {
            existingRanges = fineractClient.delinquencyRangeAndBucketsManagement().getDelinquencyRanges();
        } catch (Exception e) {
        ...
        for (int i = 0; i < DEFAULT_DELINQUENCY_RANGES.size() - 1; i++) {
            ...
            PostDelinquencyRangeResponse response = ok(
                    () -> fineractClient.delinquencyRangeAndBucketsManagement().createDelinquencyRange(postDelinquencyRangeRequest));
        ...
        PostDelinquencyRangeResponse lastResponse = ok(
                () -> fineractClient.delinquencyRangeAndBucketsManagement().createDelinquencyRange(lastRange));
        createdRangeIds.add(lastResponse.getResourceId());
    }

    public void setDefaultDelinquencyBucket() {
        try {
            List<DelinquencyBucketData> existingBuckets = fineractClient.delinquencyRangeAndBucketsManagement().getDelinquencyBuckets();
